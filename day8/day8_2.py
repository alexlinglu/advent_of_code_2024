...
for i in range(len(antennas)):
    for j in range(i + 1, len(antennas)):
        if antennas[i][0] == antennas[j][0]:
            ...
            last_first = antennas[i][1]
            while first_antinode >= 0 and not input_data[first_antinode] == "\n":
                first_lines_passed = 0
                for k in range(first_antinode, last_first):
                    if input_data[k] == "\n":
                ...
                if first_lines_passed == antenna_lines_passed:
                    ...
                    last_first = first_antinode
                    first_antinode = first_antinode - (antennas[j][1] - antennas[i][1])
                else:
                    break

            last_second = antennas[j][1]
            while second_antinode < len(input_data) and not input_data[second_antinode] == "\n":
                second_lines_passed = 0
                for k in range(last_second, second_antinode):
                    if input_data[k] == "\n":
                ...
                if second_lines_passed == antenna_lines_passed:
                    ...
                    last_second = second_antinode
                    second_antinode = second_antinode + (antennas[j][1] - antennas[i][1])
                else:
                    break
    antinode_map = antinode_map[0:antennas[i][1]] + "#" + antinode_map[antennas[i][1]+1:]
print(antinode_map)
