#implements ineffienct breadth-first search (can at least be easily reworked to find all possible trails)
import re
import math
...
r = re.compile(r"0")
possible_routes = [((math.floor(m.start() / line_size), m.start() % line_size), (0, 0), (math.floor(m.start() / line_size), m.start() % line_size)) for m in r.finditer(input_data)] #stores nodes with position and previous direction
input_data_map = []
...
while len(possible_routes) > 0:
    ...
    starting_position = possible_routes[0][2]
    if input_data_map[position[0]][position[1]] == "9" and (starting_position, position) not in trails:
        trails.append((starting_position, position))
        total += 1
    else:
        ...
        for direction in directions:
            if input_data_map[position[0] + direction[0]][position[1] + direction[1]] != "\n" and int(input_data_map[position[0] + direction[0]][position[1] + direction[1]]) == int(input_data_map[position[0]][position[1]]) + 1:
                possible_routes.append(((position[0] + direction[0], position[1] + direction[1]), (direction[0] * -1, direction[1] * -1), possible_routes[0][2]))
